...
public class CustomExcelView extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request,
		...
		row.createCell(0).setCellValue("연습용 엑셀문서 입니다.");
		HSSFRow row1 = sheet.createRow(1);
		row1.createCell(0).setCellValue("총 매입 금액");
		row1.createCell(1).setCellValue(buy);
		
		HSSFRow row2 = sheet.createRow(2);
		row2.createCell(0).setCellValue("총 매출 금액");
		row2.createCell(1).setCellValue(sell);
		
		HSSFRow row3 = sheet.createRow(3);
		row3.createCell(0).setCellValue("총 매출 이익");
		row3.createCell(1).setCellValue(profit);
		
		// 파일이 다운로드 되는 경우의 응답컨텐츠 타입 설정하기
		response.setContentType("application/octet-stream");
		// 다운로드되는 첨부파일 이름을 설정하기
